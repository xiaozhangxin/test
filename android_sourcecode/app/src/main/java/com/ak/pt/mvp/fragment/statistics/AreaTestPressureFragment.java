...
import com.ak.pt.mvp.adapter.AreListAdapter;
...
public class AreaTestPressureFragment extends BaseFragment<IPtRankView, PtRankPresenter> implements IPtRankView {
    ...
    public static AreaTestPressureFragment newInstance(AppPermissionsBean permissionsBean) {
    ...
    @Override
    public void OnGetAreaTestPressure(List<AreaPressureBean> data) {
        if (page == 1) {
            adapter.clear();
    ...
    @OnClick({R.id.ivLeft, R.id.tvStartTime, R.id.tvEndTime, R.id.ok, R.id.tvDepartment})

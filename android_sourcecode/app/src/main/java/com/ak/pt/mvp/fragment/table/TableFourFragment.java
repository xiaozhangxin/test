...
public class TableFourFragment extends BaseFragment<IAllTableView, AllTablePresenter> implements IAllTableView {
    Unbinder unbinder;
    ...
    public static TableFourFragment newInstance() {
        Bundle args = new Bundle();
        TableFourFragment fragment = new TableFourFragment();
    ...
    private void refresh() {
        map.put("page", page + "");
        getPresenter().getDoorForm(userBean.getStaff_token(), map);
    }
    ...
    public void initData() {
        ...
        map.put("type", tvType.getText().toString());
        refresh();
    }
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.ivLeft:
                ...
                break;
            case R.id.tvType:
                showSingleDialog(new String[]{"无门套", "装饰公司门套", "家装+保利门套", "通用门套", "无门套安装率", "装饰公司门套安装率", "家装+保利门套安装率", "通用门套安装率"});
                break;
    ...
    public void showSingleDialog(final String[] items) {
        DialogUtil.showSingleChoiceDialog(tvType, null, items, (dialog, which) -> {
            page = 1;
            String content = items[which];
            tvType.setText(content);
            adapter.setType(content);
            switch (content) {
                case "无门套":
                case "无门套安装率":
                    map.put("type", "无门套");

...
public class InterviewAddFragment extends BaseFragment<IInterviewView, InterviewPresenter> implements IInterviewView {
    ...
    private String type;
    private InterviewBean data;
    private ArrayList<FixFileBean> fileList;

    private AppPermissionsBean permissionsBean;

    public static InterviewAddFragment newInstance(InterviewBean bean, String type, AppPermissionsBean permissionsBean) {
    ...
    public int getRootViewId() {
        return R.layout.fragment_add_interview;
    }
    ...
    public void initUI() {
        ...
        if ("0".equals(type)) {
            ...
            tvTitle.setText(R.string.interview_add);

        } else {
            tvTitle.setText(R.string.interview_update);
                ...
                if (TextUtils.isEmpty(file_id)) {
                    ...
                    builder.setPositiveButton(getString(R.string.sure), new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            getPresenter().deleteAreaInterviewFile(userBean.getStaff_token(), map);
                            adapter.remove(data);
                        }
                    });
    ...
    public void onUploadFiles(String[] data) {
        ...
        if ("0".equals(type)) {
            ...
            getPresenter().insertAreaInterview(userBean.getStaff_token(), map);
        } else {
            map.put("areaList", toJson);
            getPresenter().updateAreaInterview(userBean.getStaff_token(), map);
        }
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.ok:
                ...
                map.clear();
                map.put("interview_time", mtvOne);
                map.put("shop_name", tvTwo.getText().toString());
                map.put("shop_serve", tvThree.getText().toString());
                map.put("shop_pressure", tvFour.getText().toString());
                map.put("shop_status", tvFive.getText().toString());
                map.put("shop_need", tvSix.getText().toString());
                map.put("shop_suggest", tvSeven.getText().toString());
                map.put("remark", tvEight.getText().toString());
                map.put("is_select", "0");
                map.put("is_app", "1");
                ...
                switch (type) {
                    ...
                    case "1"://编辑
                        map.put("operation", "1");
                        map.put("interview_id", data.getInterview_id());
                        for (FixFileBean s : allData) {

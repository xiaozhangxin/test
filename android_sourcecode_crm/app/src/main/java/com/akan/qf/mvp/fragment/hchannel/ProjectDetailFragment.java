...
public class ProjectDetailFragment extends BaseFragment<ICompanyView, CompanyPresenter> implements ICompanyView {
    ...
    public static ProjectDetailFragment newInstance(String detail_id, PermissionsBean permissionsBean) {
    ...
    @Override
    public void initUI() {
        tvTitle.setText(R.string.hvac_company_details);
        ...
        switch (v.getId()) {
            ...
            case R.id.tvRight:
                switch (tvRight.getText().toString()) {
                    case "编辑":
                        startProjectAddFragment(bean, "1", permissionsBean);
                        break;
                    case "删除":
                        toDelete();
                        break;

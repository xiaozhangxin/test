...
public class AnalysisFragment extends BaseFragment<IDispatchView, DispatchPresenter> implements IDispatchView {
    ...
    TextView tvFive;
    private List<SaleDataContrastBean> list;
    ...
    public static AnalysisFragment newInstance(PermissionsBean permissionsBean) {
    ...
    @Override
    public void initUI() {
        tvTitle.setText("销量分析记录");
        list = new ArrayList<>();
        recycleView.setLayoutManager(new LinearLayoutManager(context));
        adapter = new AnalysisListAdapter(context, list);
        recycleView.setAdapter(adapter);
        adapter.setOnItemClickListener(new RecyclerArrayAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(int position) {
                startAnalysisDetailFragment(adapter.getItem(position));
            }
        });
    ...
    private void refresh() {
        ...
        map.put("page", page + "");
        getPresenter().getSaleDataContrastList(userBean.getStaff_token(), map);
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.ivLeft:
                ...
                break;
            case R.id.tvOne:
                showOneList(new String[]{"区域销售分析", "大区销售分析", "南北方销售分析"}, 1);
                break;
            case R.id.tvTwo:

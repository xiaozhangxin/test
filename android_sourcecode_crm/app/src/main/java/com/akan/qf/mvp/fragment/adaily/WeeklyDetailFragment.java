...
public class WeeklyDetailFragment extends BaseFragment<IDailyView, DailyPresenter> implements IDailyView {
    ...
    public static WeeklyDetailFragment newInstance(String daily_id, PermissionsBean permissionsBean) {
    ...
    @Override
    public void initUI() {
        tvTitle.setText(R.string.weekly_detail);
    }
    ...
    public void onGetDailyDetail(DailyBean data) {
        ...
        if (dailyAuditBeans.size() > 0) {
            for (int i = 0; i < dailyAuditBeans.size(); i++) {
                audit = audit + dailyAuditBeans.get(i).getStaff_name() + "：" + dailyAuditBeans.get(i).getAudit_content() + "  " + dailyAuditBeans.get(i).getCreate_time() + "\n\n";
            }
    ...
    private void toDelete() {
        ...
        builder.setMessage(R.string.sure_delete_weekly);
        builder.setPositiveButton(getString(R.string.ok), new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                map.clear();

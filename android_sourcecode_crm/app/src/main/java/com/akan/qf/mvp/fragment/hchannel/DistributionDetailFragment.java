...
public class DistributionDetailFragment extends BaseFragment<INetworkView, NetworkPresenter> implements INetworkView {
    ...
    public static DistributionDetailFragment newInstance(String detail_id, PermissionsBean permissionsBean) {
    ...
    @Override
    public void initUI() {
        tvTitle.setText(R.string.distribution_network_details);
    ...
    private void refresh() {
        ...
        getPresenter().getDistributionApply(userBean.getStaff_token(), map);
    }
    ...
    public void OnGetDistributionApply(NetworkBean data) {
        ...
        if (trackList.size() > 0) {
            for (int j = 0; j < trackList.size(); j++) {
                ...
                track.append(trackListBean.getTrack_remark() + "  "
                        + trackListBean.getTrack_create_time() + "\n\n");
            }
        ...
        switch (v.getId()) {
            ...
            case R.id.tvRight:
                switch (tvRight.getText().toString()) {
                    case "编辑":
                        startDistributionAddFragment(bean, "1", permissionsBean);
                        break;
                    case "删除":
                        toDelete();
                        break;
    ...
    private void toDelete() {
        ...
        builder.setPositiveButton(getString(R.string.ok), new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                getPresenter().deleteDistributionApply(userBean.getStaff_token(), map);
            }
        });
    ...
    private void startDayReviewFragment(String type) {
        ...
        fragment.setOnReviewClickListener(new ReviewFragment.OnReviewClickListener() {
            ...
            public void ok(String content) {
                ...
                map.put("operation", "3");
                map.put("module_id", permissionsBean.getMenu_id());
                getPresenter().auditDistributionApply(userBean.getStaff_token(), map);
            }
        });
    ...
    public void OnDeleteDistributionApply(String data) {
        EventBus.getDefault().post(new FirstEventFilter("distribution_delete"));
        ToastUtil.showToast(context.getApplicationContext(), data);
        finish();
    }
